...
impl CounterLowering { pub fn new() -> Self { Self } }
impl Default for CounterLowering { fn default() -> Self { Self::new() } }
...
impl Lowering for CounterLowering {
    ...
    fn reply_from_effects(&self, operation: &Self::Operation, effects: &[SemaEffect]) -> Self::Reply {
        match operation {
...
fn first_wrote_for(effects: &[SemaEffect], wanted: SemaOperation) -> u64 {
    effects.iter().find_map(|e| match (e.operation, &e.outcome) {
...
impl Default for CounterEngine { fn default() -> Self { Self::new() } }
...
impl SemaEngine for CounterEngine {
    ...
    fn execute_atomic(&mut self, ops: Vec<SemaOperation>) -> Result<Vec<SemaEffect>, Self::Error> {
        if self.poisoned { return Err(PoisonError); }
        let effects: Vec<SemaEffect> = ops.into_iter().map(|op| match op {
            SemaOperation::Assert => SemaEffect::new(op, SemaEffectOutcome::Wrote { rows_written: 1, rows_matched: 0 }),
            SemaOperation::Mutate | SemaOperation::Retract => SemaEffect::new(op, SemaEffectOutcome::Wrote { rows_written: 1, rows_matched: 1 }),
            SemaOperation::Match => SemaEffect::new(op, SemaEffectOutcome::Read { rows_read: 7 }),
            SemaOperation::Subscribe => SemaEffect::new(op, SemaEffectOutcome::Stream { subscription_token: 42 }),
            SemaOperation::Validate => SemaEffect::new(op, SemaEffectOutcome::Validated { predicate_held: true }),
        }).collect();
